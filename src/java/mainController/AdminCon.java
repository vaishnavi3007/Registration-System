...
public class AdminCon {
    ...
    //return rollno for course registration report
    public static List courseRegReport(String programme,int shift,int year,String reg_term_year)
    {
        ...
        String hql;
            ...
            session=sf.openSession();
             ...
             qr = session.createQuery("SELECT regno FROM Course_reg WHERE program =:programme and year =:year and shift=:shift and reg_term_year=:reg_term_year and reg_exmt='n' and reg_can='n' GROUP BY regno");
    ...
    //It updates as follows : 1st year->2nd year and 2nd year->3rd year
    public static boolean autoUpdateYear()
    {
        ...
        Query q=session.createQuery("update Student_data set year=3 where year=2");
        int res=q.executeUpdate();
        
        q=session.createQuery("update Student_data set year=2 where year=1");
        int res2=q.executeUpdate();
        session.getTransaction().commit();
        session.close();
        //sf.close(); 
        System.out.print("_______________________________res="+res+"___--res2 = "+res2);
        if(res!=0 || res2!=0)
            return true;
        else
            return false;       
   ...
   //deletes the course registration of particular student.
    public static int courseRegReset(String roll_no)
    {
       ...
       //return rollno for course registration report
    public static List examRegReport(String programme,int shift,int ex_reg_year,String reg_term_year)
    {
        ...
        String hql;
            ...
            session=sf.openSession();
             ...
             qr = session.createQuery("SELECT regno FROM Exam_reg WHERE prog =:programme and ex_reg_year =:ex_reg_year and shift=:shift and ex_reg_date=:ex_reg_date and  ex_exmt=0 and ex_cancel=0 GROUP BY regno");
                 qr.setString("programme", programme);
                 qr.setInteger("ex_reg_year", ex_reg_year);
                 qr.setInteger("shift",shift);
                 qr.setString("ex_reg_date", reg_term_year);
                    //query.list() will give all roll no's that are registered of that department.
                    list1=qr.list(); 
    ...
    //It updates as follows : ODD -> EVN  and EVN -> ODD
